...
function setupBlocks() {
    ...
    heights.length = [];
    // setting the width of a column or card to 16rem 
    colWidth = parseInt(window.getComputedStyle(document.body).getPropertyValue('font-size')) * 16;
    // grabbing all cards
    blocks = document.querySelectorAll('.card');
    // getting window width and setting the margin to be equal to one rem
    ...
    // array to keep track of how many cards are in each column
    rows = new Array(colCount);
    ...
    if (blocks.length >= colCount) {
        for (let i = 0; i < colCount; i++) {
            ...
            // identifying which column this card belongs to
            blocks[i].setAttribute('data-column', i);
            rows[i] = [1];
            blocks[i].setAttribute('data-row', rows[i].length - 1);
        ...
        blocks[0].setAttribute('data-column', 0);
        rows[0] = [1];
        blocks[0].setAttribute('data-row', rows[0].length - 1);
    ...
    for (let i = colCount; i < blocks.length; i++) {
        ...
        // identifying which column this card belongs to
        blocks[i].setAttribute('data-column', index);
        rows[index].push(1);
        blocks[i].setAttribute('data-row', rows[index].length - 1);
...
// handles the change in size of cards when a new picture is displayed
function changeBlocks(card){
    const col = parseInt(card.getAttribute('data-column'));
    ...
    // console.log(height);
    const cards = document.querySelectorAll(`[data-column="${col}"]`);
    if(row + 1 < cards.length) {
        for(let i = row + 1; i < cards.length; i++){
            cards[i].style.top = `${top + height + margin}px`;
            top += height + margin;
...
function createCard(data) {
    ...
    container100.appendChild(card);
     ...
     (card__image, data.images, colWidth, data._id);
...
function changePicture(carousel, images) {
    ...
    rightButton.addEventListener('click', function () {
        ...
        changeBlocks(this.parentNode.parentNode);
    });

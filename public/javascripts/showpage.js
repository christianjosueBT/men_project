...
function changeLeft(images) {
    for(let i = 0; i < length; i++){
        if(parseFloat(images[i].style.left) <= 0 && parseFloat(images[i].style.left) >= -images[i].scrollWidth) {
...
function changeRight(images) {
    ...
    for(let i = 0; i < length; i++){
        if(parseFloat(images[i].style.left) <= width && parseFloat(images[i].style.left) >= (width-images[i].scrollWidth)) {
            ...
            if(i === (length - 1) && parseFloat(rightMost.style.left) === (width - rightMost.scrollWidth)) return;
            else if(parseFloat(rightMost.style.left) === (width - rightMost.scrollWidth)){
                ...
                const L0 = rightMost.scrollWidth - (width - parseFloat(rightMost.style.left));
                console.log('L0: ', L0);
                for(let j = 0; j < length; j++){
                    const L = parseFloat(images[j].style.left);
                    images[j].style.left = `${L-L0}px`;
...
function loadImages(images) {
    ...
    let str = `https://res.cloudinary.com/christianjosuebt/image/upload/q_auto,f_auto,fl_lossy,h_${Math.round(height * pixelRatio)}/coffeeShops`;
